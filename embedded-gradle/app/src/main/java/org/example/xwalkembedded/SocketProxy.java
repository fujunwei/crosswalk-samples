...
public class SocketProxy {
    final static private String TAG = "SocketProxy";
    ...
    public void startProxy() throws IOException {
        ...
        new Thread() {
            public void run() {
                ...
                byte[] remote_reply = new byte[1024 * 10];
                while (true) {
                    try {
                        //--------------------------------------
                        ...
                        if ((bytes_read = in_localSocket.read(local_request)) != -1) {
                        ...
                        //--------------------------------------
//                        getLocalURL("http://www.zhangxinxu.com/study/media/cat.mp4");
                        remoteHost = (String) parseRequestHeader(buffer).get("Remote-Host");
                        remotePort = Integer.parseInt((String) parseRequestHeader(buffer).get("Remote-Port"));
                        ...
                        //        && buffer.contains("\r\n\r\n")) {
                            ...
                            if (remotePort == -1) {
                                buffer = buffer.replace(":" + local_ip_port, "");
                                remotePort = HTTP_PORT;
                            } else {
                                buffer = buffer.replace(":" + local_ip_port, ":"+ remotePort);
                            }
                        //}
                        Log.d(TAG, remoteHost + " ==== " + buffer);
                        //originalURI.getPort());//使用默认端口
    ...
    public static HashMap<String,Object> parseRequestHeader(String headers_str) throws IOException{
        ...
        while((line = bur.readLine()) != null){
            ...
            if(line_index == 0){
                ...
                headerList.put(h.getName(), h.getValue());
            }
            line_index ++;
        }
        bur.close();
