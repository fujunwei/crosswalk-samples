...
public class StreamProxySeek implements Runnable {
    ...
    public void run() {
        ...
        while (isRunning) {
            try {
                Log.d(LOG_TAG, "==== wait media player client");
    ...
    private HttpResponse download(String url) {
        ...
        SingleClientConnManager mgr = new MyClientConnManager(httpClient.getParams(),
                registry);
        DefaultHttpClient http = new DefaultHttpClient(mgr, httpClient.getParams());
    ...
    private void processRequest(HttpRequest request, Socket client)
        ...
        httpString.append("\n");
        Log.d(LOG_TAG, "==== headers done " + httpString.toString());
